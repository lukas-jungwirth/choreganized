...
133. **An underline in cook mode is a button, and the amount appears against the word** (→ SPEC
...
135. **Feedback is a row first and an issue second** (→ SPEC §6, `server/services/feedback.ts`,
...
136. **The build id comes from a build arg, and `dev` is what dev honestly reports**
