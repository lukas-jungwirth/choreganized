...
- **Photos**: `sharp@0.35.3` is the one new dependency (→ [DECISIONS #60–62](../DECISIONS.md)).
...
- **A real bug the walkthrough caught** (→ [#63](../DECISIONS.md)): Drizzle leaves column names
...
- **What the code review caught after that**, all fixed here: adapter-node's 512K
  `BODY_SIZE_LIMIT` would have rejected every real photo _in production only_ (→ [#70]) — the
  ...
  nothing and reported success (→ [#71]); searching the plan sheet while a recipe was selected
...
- **Deviations from the design**, all logged: a ••• on planned meal rows (#64), arrow reorder
  instead of drag (#65), our pill `Toggle` for the shopping switch (#66), "Copy recipe" where
  Web Share is missing (#68), `chef-hat` rather than the mockup's music note on "Start cook
  mode" (#69).
